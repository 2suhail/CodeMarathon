...
def main():
    ...
    def start_timer():
        ...
        if reps % 8 ==0:
            ...
            timer_label.config(text="Break", bg=YELLOW, fg=RED, font=(FONT_NAME, 20, "bold"))

        elif reps % 2 == 0:
            count_down(short_break_sec)
            timer_label.config(text="Break", bg=YELLOW, fg=PINK, font=(FONT_NAME, 20, "bold"))
        else:
            count_down(work_sec)
            timer_label.config(text="Work", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 20, "bold"))
            check_mark.config(text="∎∎")
    # ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
